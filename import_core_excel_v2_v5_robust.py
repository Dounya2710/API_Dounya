...
def qa_check_location_consistency(data, strict=False):
    """
    Checks if KLCD_Location.location_id values are all present in Location.location_id.
    Returns set of valid location_ids (from Location) or None if sheets missing.
    """
    if "Location" not in data or "KLCD_Location" not in data:
        print("[QA] Location consistency: skipped (missing Location/KLCD_Location sheet)")
        return None

    loc_ids = set_of_ints(data["Location"]["location_id"])
    klcd_loc_ids = set_of_ints(data["KLCD_Location"]["location_id"])

    orphans = klcd_loc_ids - loc_ids
    print(f"[QA] Location ids: {len(loc_ids)} | KLCD_Location ids: {len(klcd_loc_ids)}")
    print(f"[QA] Orphans in KLCD_Location (not in Location): {len(orphans)}")

    if strict and orphans:
        sample = list(sorted(orphans))[:10]
        raise ValueError(f"[QA] Incoherence: {len(orphans)} orphan location_id in KLCD_Location. Sample: {sample}")

    return loc_ids


def get_pk_cols(conn, table):
    """Return primary key column names for a table (public schema)."""
...
def table_columns(conn, table):
    """Return set of column names for a table (public schema)."""
    with conn.cursor() as cur:
        cur.execute("""
            SELECT column_name
...
def bulk_upsert(conn, table, columns, rows, conflict_cols):
    """
    Robust upsert helper using INSERT ... ON CONFLICT.
    - If conflict_cols don't match any UNIQUE/PK constraint => retry with PK columns if available.
    - Plain insert fallback uses ON CONFLICT (PK) DO NOTHING to avoid duplicate key crashes.
    """
    if not rows:
    ...
    def _insert_plain_do_nothing_if_pk():
        if pk_cols:
            conflict_sql = ", ".join(f'"{c}"' for c in pk_cols)
            sql_plain = f'INSERT INTO {table} ({cols_sql}) VALUES %s ON CONFLICT ({conflict_sql}) DO NOTHING;'
        else:
    ...
    # If no conflict columns provided, just insert (but avoid duplicates if PK exists)
    if not conflict_cols:
        return _insert_plain_do_nothing_if_pk()

    def _build_sql(conf_cols):
        conflict_sql = ", ".join(f'"{c}"' for c in conf_cols)
        update_cols = [c for c in columns if c not in conf_cols]
        ...
        return f'INSERT INTO {table} ({cols_sql}) VALUES %s {on_conflict};'

    sql = _build_sql(conflict_cols)
    ...
    try:
        ...
        # conflict_cols don't match an existing UNIQUE/PK constraint
        conn.rollback()
        if pk_cols and set(pk_cols) != set(conflict_cols):
            try:
                sql2 = _build_sql(pk_cols)
                with conn.cursor() as cur:
                    execute_values(cur, sql2, rows, page_size=1000)
                conn.commit()
                print(f"[OK] {table}: {len(rows)} rows (upsert via PK)")
                return
            except Exception:
                conn.rollback()
        # last resort: insert and ignore duplicates on PK if possible
        return _insert_plain_do_nothing_if_pk()
    except psycopg2.errors.UniqueViolation:
        # duplicates already present: ignore them and continue
        conn.rollback()
        return _insert_plain_do_nothing_if_pk()
...
def main():
    ...
    # QA checks (do not change DB schema; only report / optionally fail)
    strict_qa = env("STRICT_QA", "0") == "1"
    valid_location_ids = qa_check_location_consistency(data, strict=strict_qa)
...
# 5) Location (optional)
# Large sheet; disabled by default. Enable with env IMPORT_LOCATION=1
    if env("IMPORT_LOCATION", "0") == "1" and "Location" in data:
        df = data["Location"]
        rows = []
        for _, r in df.iterrows():
            ...
            rows.append((
                ...
                int(to_int_or_none(pick(r, "admin_level")) or 0),
        ...
        print("[SKIP] location: 0 rows")


    # 6) KLCD_Location (optional)
    # Disabled by default. Enable with env IMPORT_KLCD_LOCATION=1
    # If Location sheet was not imported, we can still filter based on valid_location_ids from QA.
    if env("IMPORT_KLCD_LOCATION", "0") == "1" and "KLCD_Location" in data:
        ...
        for _, r in df.iterrows():
            ...
            if kid is None or lid is None:
                continue
            if valid_location_ids is not None and lid not in valid_location_ids:
                continue
            rows.append((kid, lid, None))  # share_pct unknown -> NULL
        rows = dedupe_rows(rows, [0, 1])
        bulk_upsert(conn, "klcd_location",
                    ["klcd_id", "location_id", "share_pct"],
                    rows, ["klcd_id", "location_id"])
    else:
        print("[SKIP] klcd_location: 0 rows")
    ...
    if "Action_Implementer" in data:
        ...
        allowed_roles = {"Leader", "Co-implementer"}
        invalid_roles = []
        ...
        for _, r in df.iterrows():
            ...
            if role not in allowed_roles:
                invalid_roles.append((aid, iid, role))
                continue
            rows.append((aid, iid, role))

        if invalid_roles:
            print(f"[QA] Action_Implementer: {len(invalid_roles)} role_code invalides (ex: {invalid_roles[:5]})")

        # Deduplicate on PK (action_id, institution_id). If duplicates exist with different roles,
